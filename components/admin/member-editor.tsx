...
type Member = {
  ...
  links: { id: string; title: string; workDate: string }[];
};
    ...
    <div className="space-y-5">
      <Card>
        ...
        >
          <label className="block">
            ...
            <input name="displayName" defaultValue={member.displayName} required className="field" />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs text-mute">ยูสเซอร์เนม</span>
            <input name="username" defaultValue={member.username} required className="field" />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs text-mute">ช่องทางติดต่อ</span>
            <input name="contact" defaultValue={member.contact} className="field" />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs text-mute">สถานะ</span>
            <select name="status" defaultValue={member.status} className="field">
          ...
          <label className="block">
            ...
            <input name="warnCount" type="number" min={0} defaultValue={member.warnCount} className="field" />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs text-mute">รหัสผ่านใหม่ (เว้นว่างถ้าไม่เปลี่ยน)</span>
            <input name="password" type="password" minLength={6} className="field" />
          </label>
          <label className="sm:col-span-2 block">
            <span className="mb-1 block text-xs text-mute">โน้ตแอดมิน</span>
            <textarea name="adminNote" rows={3} defaultValue={member.adminNote} className="field" />
          </label>
          <label className="sm:col-span-2 block">
      ...
      <Card padded={false}>
        ...
        {member.links.length === 0 ? (
          ...
          <TableShell caption="ลิงก์ล่าสุดของสมาชิก">
            <thead>
              <tr>
                <th>ลิงก์</th>
                <th>วันที่</th>
            ...
            <tbody>
              {member.links.map((link) => (
                <tr key={link.id}>
                  <td className="max-w-xs truncate">{link.title}</td>
                  <td className="whitespace-nowrap text-mute">{link.workDate}</td>
                </tr>
              ))}
            </tbody>
          </TableShell>
