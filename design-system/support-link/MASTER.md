...
**Color Notes:** Product is dark OLED + Flame CTA across admin, member, and public UI.

### Product override (Support Link)

Shopee Affiliate keeps Flame as the only filled CTA. Dark OLED is the default for the whole product (not admin-only). Swiss layout and checklist still apply.

| Role | Hex | CSS Variable |
|------|-----|--------------|
| Canvas | `#071018` | `--canvas` |
| Card | `#102433` | `--paper-2` |
| Ink | `#F4F7FB` | `--ink` |
| Mute | `#C5D0DC` | `--mute` |
| Border | `#2C4558` | `--border` |
| Flame CTA | `#EE4D2D` | `--flame` |
| Gold wait | `#F0C14A` | `--gold` |
| Jade success | `#3DCE9A` | `--jade` |
| Sky info | `#5B9CFF` | `--sky` |
| On gold fill | `#171717` | `--teak` |

Typography stays Anuphan (Thai body) + Taviraj (display, rare) + IBM Plex Mono (clocks). Icons: Lucide outline 20px, never emoji. Gold filled controls use `--teak` text. Toasts and clicks may play short Web Audio cues; honor mute + `prefers-reduced-motion`.
